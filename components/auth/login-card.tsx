...
function LoginButton({ slug, name, meta, color }: { slug: "james" | "cory"; name: string; meta: string; color: string }) {
  return (
    <form action={signInAs.bind(null, slug)}>
      <button className="group flex w-full items-center gap-3.5 rounded-[10px] border-[1.5px] border-border bg-card px-4 py-3.5 text-left transition hover:-translate-y-px hover:border-accent hover:bg-[var(--accent-soft)]" type="submit">
...
export function LoginCard({ error }: { error?: string }) {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-app px-5 py-10" style={{ backgroundImage: "radial-gradient(ellipse 60% 40% at 30% 0%, rgba(79, 91, 213, 0.04), transparent), radial-gradient(ellipse 50% 30% at 70% 100%, rgba(147, 51, 234, 0.03), transparent)" }}>
      <div className="mb-14 flex items-center gap-3">
        <BrandMark className="h-8 w-8" />
        <div className="text-base font-bold tracking-[0.05em]">PROOFBRIDGE</div>
      </div>
      <div className="w-full max-w-[440px] rounded-[16px] border border-border bg-card px-9 py-10 shadow-[0_4px_16px_rgba(0,0,0,0.04)]">
        ...
        <p className="mb-7 text-[13.5px] leading-[1.55] text-secondary">Pick your account to continue. Your view is tailored to what&apos;s waiting on you.</p>
        ...
        <div className="rounded-lg border border-border bg-subtle px-3 py-3 text-center text-[13px] font-medium text-secondary">Seeded demo auth with Supabase password login.</div>
        <div className="mt-8 text-center text-[11.5px] text-tertiary">Not an account holder? Talk to James or Cory.</div>
      </div>
    </div>
  );
}
